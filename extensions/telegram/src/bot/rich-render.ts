...
import type { RichBlock, RichBlockListItem, RichText } from "grammy/types";
import { markdownInlineCodeDelimiters, markdownPreAffixes } from "./inbound-text-entities.js";
...
// Blocks-only input: callers hand over an unvalidated inbound payload, so the renderer takes
// the loose shape it actually reads instead of forcing a RichMessage cast at every call site.
...
export function renderRichCommandText(rich: { blocks?: unknown }): string {
  const blocks = Array.isArray(rich.blocks)
    ...
    : [];
  return renderRichMessageToText({ blocks }, "plain");
}
...
function renderRichText(node: unknown, mode: RichRenderMode, depth: number): string {
  ...
  // An inline button carries its label under .button.text, not .text; read it here where the
  // depth budget lives so renderInline stays a pure markup switch.
  const label = wrap.type === "button" ? asLoose(wrap.button).text : wrap.text;
  const inner = label != null ? renderRichText(label, mode, depth + 1) : "";
...
function renderBlock(block: RichBlock, mode: RichRenderMode, depth: number): string {
  ...
  switch (type) {
    ...
    case "buttons":
      // Button labels are the only user-visible text on this block; one per line, no markup
      // (a button is not actionable from a flattened transcript).
      return (Array.isArray(loose.buttons) ? loose.buttons : [])
        .map((button) => renderRichText(asLoose(button).text, mode, depth + 1))
        .filter(Boolean)
        .join("\n");
    case "mathematical_expression": {
