...
describe("renderRichMessageToText — inline code shares the entity-path affixes", () => {
  // rich-render and renderTelegramTextEntities must produce identical code spans for the same
  // content; they share markdownInlineCodeDelimiters so a fix on one path cannot skip the other.
  it("pads when the content edges on a backtick", () => {
    expect(renderInline({ type: "code", text: "`x`" })).toBe("`` `x` ``");
  });

  it("pads when the content edges on a space", () => {
    // The padding space is what CommonMark strips, so the literal edge spaces survive.
    expect(renderInline({ type: "code", text: " x " })).toBe("`  x  `");
  });

  it("does not pad all-space content", () => {
    expect(renderInline({ type: "code", text: "   " })).toBe("`   `");
  });
});

describe("renderRichMessageToText — buttons (Bot API 10.2)", () => {
  it("renders each button label on its own line", () => {
    expect(
      renderBlock({
        type: "buttons",
        buttons: [
          { text: "Copy result", copy_text: { text: "result" } },
  ...
  it("keeps button labels free of markup in both modes", () => {
    const buttons = { type: "buttons", buttons: [{ text: [{ type: "bold", text: "Go" }] }] };
    expect(renderBlock(buttons, "markdown")).toBe("**Go**");
    expect(renderBlock(buttons, "plain")).toBe("Go");
  });

  it("renders an inline button label from .button.text", () => {
    expect(renderInline({ type: "button", button: { text: "Retry" } })).toBe("Retry");
  });

  it("degrades to empty on a malformed buttons payload instead of throwing", () => {
    expect(renderBlock({ type: "buttons" })).toBe("");
    expect(renderBlock({ type: "buttons", buttons: "nope" })).toBe("");
    expect(renderInline({ type: "button" })).toBe("");
  });
});
