...
// Dependencies injected by the gateway (avoids direct src/ cross-boundary imports)
export type TelegramInjectDeps = {
  loadConfig: () => OpenClawConfig;
  getBearerToken: (req: IncomingMessage) => string | null | undefined;
  saveMediaBuffer: (
    buffer: Buffer,
    contentType?: string,
    subdir?: string,
...
function getTelegramInjectConfig(cfg: OpenClawConfig, accountId: string | undefined) {
...
export async function handleTelegramInjectRequest(
  ...
  // Verify token
  const token = deps.getBearerToken(req);
